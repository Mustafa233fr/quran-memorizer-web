...
function generateQuestion() {
    ...
    if (currentMode === "surah") {
        ...
        s.verses.forEach((v, v_idx) =>
            validAyahsPool.push({
                s_idx: parseInt(selectedIndex),
                v_idx: v_idx
            })
        );
    } else {
        ...
        if (currentMode === "page") {
            const p1 =
                parseInt(document.getElementById("page-start").value) || 1;
            const p2 =
                parseInt(document.getElementById("page-end").value) || 604;
...
function checkAnswers() {
    ...
    function check(label, userInput, correctAnswer, isAyah = false) {
        ...
        if (isAyah) {
            isCorrect =
                (canCorrect.includes(canUser) ||
                    canUser.includes(canCorrect)) &&
                canUser.length > 3;
